...
// Combining marks and variation selectors (Unicode category M) compose onto the preceding
// base character and occupy zero terminal columns. macOS stores accented names in decomposed
// NFD form (e.g. "cafe" + U+0301 for "café"), so a project or branch name reaching this with
// combining marks is a real, reachable case, not a contrived one.
const COMBINING_RE = /\p{M}/u;
// Default-presentation emoji render two columns wide, including BMP dingbats (check mark,
// star, high voltage) that a branch or project name can contain. Residual, approximated not
// measured: a text symbol widened only by U+FE0F, and multi-codepoint ZWJ emoji clusters.
...
async function buildWrapped(records, opts = {}) {
	...
	for (const r of records) {
		const priced = priceFn(r.tokens, r.model, r.date);
		if (!priced.modelKnown && r.model) unpriced.add(r.model);
		// Coerce both sides of the counterfactual to finite. The scan pipeline already guarantees
		// finite tokens (scan.js tokInt), so this is unreachable through the CLI; it guards the
		// module boundary so no reuse or future pricer edge case can ever render a $NaN card.
		const usd = num(priced.usd);
		const noCache = num(priceFn(withoutCache(r.tokens), r.model, r.date).usd);
